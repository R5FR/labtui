//!
//! The labtui program is a text-based UI for working with a Git repository.
...
//!
//! ## Internal Modules
//! The top-level modules of labtui can be grouped as follows:
//!
//! - User Interface
...
//!
//! ## Included Crates
//! Some crates are part of the labtui repository:
...
pub enum AsyncNotification {
	...
	///
	GitLab(asyncgitlab::AsyncGitLabNotification),
}
...
fn select_event(
	...
	rx_gitlab: &Receiver<asyncgitlab::AsyncGitLabNotification>,
	...
	let ev = match index {
		...
		3 => oper.recv(rx_gitlab).map(|e| {
			QueueEvent::AsyncEvent(AsyncNotification::GitLab(e))
		}),
...
fn set_panic_handler() -> Result<()> {
	panic::set_hook(Box::new(|e| {
		...
		shutdown_terminal();
		log_eprintln!("\nlabtui was closed due to an unexpected panic.\nPlease file an issue on https://github.com/R5FR/labtui/issues with the following info:\n\n{e}\n\ntrace:\n{backtrace:?}");
	}));
